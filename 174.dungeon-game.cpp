...
public:
#ifdef SOLUTION1
    static int calculateMinimumHP (vector<vector<int>>& dungeon, int &minhp, int value,
			int lowest, int i, int end_i, int j, int end_j) {
		value += dungeon[i][j];
		lowest = min(lowest, value);
		if (lowest <= minhp)
			return -1;
		//printf("%d,%d: %d   %d, %d, %d\n", i, j, dungeon[i][j], value, lowest, minhp);
		if (i==end_i && j==end_j) {
			minhp = lowest;
			return 0;
		}
		if (j<end_j && i<end_i) {
			if (dungeon[i][j+1]>dungeon[i+1][j]) {
				calculateMinimumHP(dungeon, minhp, value, lowest, i, end_i, j+1, end_j);
				calculateMinimumHP(dungeon, minhp, value, lowest, i+1, end_i, j, end_j);
			}
			else {
				calculateMinimumHP(dungeon, minhp, value, lowest, i+1, end_i, j, end_j);
				calculateMinimumHP(dungeon, minhp, value, lowest, i, end_i, j+1, end_j);
...
#ifdef SOLUTION1
		int sz_i=dungeon.size(), sz_j=dungeon[0].size();
		int minhp=INT_MIN;
		if (sz_i==0 || sz_j==0)
			return 0;
		calculateMinimumHP (dungeon, minhp, 0, INT_MAX, 0, sz_i-1, 0, sz_j-1);
		minhp = min(0,minhp);
		return -minhp+1;
#elif SOLUTION2
